...
1. **信息缺失**：原文缺少背景、问题场景，无法自行补充
2. **跨度过大**：原文同时涉及 3 个以上独立技术领域，难以整合
3. **内容碎片化**：原文是笔记/要点形式，缺乏连贯叙事
...
| 长句 | 单句超过 40 字（中文字符逐字计数，英文单词和数字各算 1 字） | **必须**拆分为多个短句，每句 ≤ 40 字。这是硬性要求，无例外。特别注意：枚举多个概念时容易写出长句，必须拆成多句。❌ "白色表示未访问，灰色表示已发现但引用未扫描完，黑色表示已扫描完。"（超 40 字）→ ✅ 拆为三句，每句介绍一种颜色。 |
...
*   **章节间过渡**：每个 ## 级别章节（除最后一个章节外）都必须在末尾有 1-2 句过渡句，引向下一章节的主题。无一例外——包括倒数第二个章节到"延伸思考/下一步"之间也必须有过渡。
    *   ✅ "理解了原理后，接下来看生产环境中最容易踩的坑：参数配置。"
    *   ✅ "方案设计完成，但如何验证它真的有效？"
    *   ✅ "优化完成后，回顾一下整体思路和可继续探索的方向。"（倒数第二章到结尾章节的过渡）
    *   ❌ 无过渡直接跳到下一章节
    *   ❌ 最后一个内容章节结束后直接跳到"延伸/下一步"
    *   ❌ 章节以代码块、TODO 注释或列表收尾而无过渡句（过渡句必须是正文文字，不能是注释或代码）
