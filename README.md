...
**Usage**: 获取每日洞见、探索跨学科思维模型、将抽象知识转化为行动原则、寻求认知突破时调用。
...
| `@insight-miner <领域>` | 指定领域生成洞见 |
| `@insight-miner --regenerate` | 重新生成 |
| `@insight-miner --list` | 查看历史 |
| `@insight-miner --stats` | 显示统计 |

**Output Format**: 四步深度框架（认知破壁 → 系统剖析 → 行动基石 → 行动反思）
